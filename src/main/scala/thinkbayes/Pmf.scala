...
case class Pmf[K](hist: Map[K, Double]) extends Map[K, Double] with MapLike[K, Double, Pmf[K]] {
  ...
  def prob(pred: K => Boolean) = toIterator.filter { case (k, _) => pred(k) }.map(_._2).sum
  ...
  def mapKeys[K2](f: K => K2): Pmf[K2] =
    foldLeft(Pmf.empty[K2]) { case (acc, (k, prob)) => acc + (f(k), prob) }.normalized
...
object Pmf {
  ...
  implicit def builder[K]: mutable.Builder[(K, Double), Pmf[K]] =
    new mutable.Builder[(K, Double), Pmf[K]] {
      ...
      override def result() = Pmf(hist)
      override def clear() { hist = Map.empty }
      override def +=(elem: (K, Double)) = {
        hist = hist.updated(elem._1, hist.getOrElse(elem._1, 0.0) + elem._2)
        this
      }
    }

  implicit def canBuildFrom[K]: CanBuildFrom[Pmf[K], (K, Double), Pmf[K]] =
    new CanBuildFrom[Pmf[K], (K, Double), Pmf[K]] {
      override def apply() = builder[K]
      override def apply(from: Pmf[K]) = builder[K]
    }
