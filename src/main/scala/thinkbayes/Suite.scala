...
trait Suite[H, D] {
  ...
  /** Updates each hypothesis based on the given data.
    ...
    * @return
    *   a new [[Suite]] with the updated hypotheses.
    */
  def observed(data: D): Suite[H, D]

  def observed(dataset: D*): Suite[H, D] = observedSet(dataset)

  /** Updates each hypothesis based on the given dataset. This is more efficient than calling `update` repeatedly
    * because it waits until the end to `normalize`.
    * @param dataset
